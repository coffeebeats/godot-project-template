...
const PropertyMuteBackground := preload("background_property.tres")
...
func _handle_value_change(_property: StdSettingsProperty, value: bool) -> void:
	assert(_handler is Node, "invalid state: missing handler node")
	if not _handler:
		return

	_handler.enabled = value
