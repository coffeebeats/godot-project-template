...
5. The edit checks, `godot-bridge` and the `kit:*` skills come from two agent plugins, declared in [`.claude/settings.json`](./.claude/settings.json) for Claude Code and [`.codex/config.toml`](./.codex/config.toml) for Codex. Claude Code installs them once per machine with `claude plugin install godot@godot-infra --scope project` and `claude plugin install kit@godot-plugin-kit --scope project`. Codex installs them itself once the folder is trusted, then asks you to trust the edit hook in `/hooks`. If the hook or `godot-check` goes missing, see the "Agent plugin" section of [godot-infra's README](https://github.com/coffeebeats/godot-infra#agent-plugin).
