...
- **`addons/kit/`** — Game infrastructure (git submodule; do not edit directly). Classes use the `Kit` prefix. Every value the game owns is set on a kit instance, never by editing kit: in `project/main/system.tscn` and `platform.tscn`, or on the root of a map inheriting a kit template. The screen manager is the exception, passed at runtime: `main.gd` hands it to `KitSystems.audio()`, and whoever opens a `KitAlertDialog` passes it to `open()`.
  - **`system/`** — Subsystems the `System` autoload places. Accessed via `KitSystems.audio()`, `KitSystems.input()`, `KitSystems.saves()`.
    ...
    - **`input/`** — UI navigation, cursor management, gamepad/Steam Input support. `actions/` holds the menu action sets.
  ...
  - **`ui/`** — Shared UI: screen transitions (fade, slide), input glyphs, modals, tooltips, world-space trackers. `hud/` is the HUD layer, its anchor and group, and the stock elements; `feel/` is camera shake, hit-stop and flash.
