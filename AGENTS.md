...
- **Adding a screen** — Create a `.tscn` scene and `.gd` script. Create a `StdScreen` resource (`.tres`) pointing to the scene with transition config. Export or preload the resource in `main.gd`. Navigate via `Main.screens().push()`, `.replace()`, `.pop()`, or `.reset()`. To drive the screen with input actions, put a `StdScreenPusher` scene in the `attachment_scenes` of the screen it is opened **from**, never in the scene itself and never on the screen it pushes (see `project/menu/settings/pusher.tscn`, attached to `project/main/menu/screen.tres`). Attachments mount into the opener's overlay, so they are unaffected by that screen's `pause_when_covered` and the scene stays runnable on its own. A pusher attached to the screen it pops crashes the engine: the manager frees the overlay synchronously during the pop, destroying the pusher while its own `_unhandled_input` is still on the stack.
