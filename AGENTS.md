...
- **`project/`** — Game-specific code and assets.
  ...
  - **`input/`** — The game's action sets, and its Steam Input manifest resource (`steam_in_game_actions.tres`), which `addons/std/input/steam/write_in_game_actions.gd` writes as `game_actions_480.vdf` at export; the file is not committed.
