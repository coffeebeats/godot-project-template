...
everything that can be judged without a live process; this covers the rest — what is on
screen, what the scene tree looks like right now, and what the game just logged.
...
`python` on `PATH` is not always a program — see [below](#driving-it-from-a-windows-shell).

Node paths — `tree --path`, `screenshot --node` — are relative to `/root`, so it is
`--path Main`, not `--path /root/Main`. The absolute form still works, but an MSYS shell
mangles it before the tool sees it, so the relative one is what the examples use.

This is why there is no third-party Godot MCP: what needs a live process is a few
hundred lines we own, with no tool schemas in context and no addon to track engine
releases.

### How it stays out of a release build, and out of the way in a debug one
...
2. The node listens only when handed a port — `--bridge-port <N>` after `--`, or
...
`ProjectMap`, which every inherited map gets for free. This is also how a prototype's
simulation will expose intent injection and a state dump without the template ever
depending on it.
...
fails with `Invalid named index 'Main' for base type Object` — the bridge reaches no
project API at all. The singleton half is not obvious, because `ResourceLoader` and
`OS` read like built-ins; they are not, and `ResourceLoader.load(...)` fails the same
way until it is bound.
...
alone was tried: after one push and pop the app reports `settled=false` forever and
`wait` times out on a game that is sitting there, idle and usable.
...
A client that waits for "settled" acts on the splash, and everything it then reports is
about a scene that is ignoring it. `Main._is_booted()` is what excludes the loading and
splash screens, which is also why that lives in `project/` and not in `system/`.
...
bridge. `launch` reaps first: a graceful `quit` over the port, then the pid it recorded
for that port — after checking the pid still names a Godot process, since pids are
recycled. A port held by an editor-launched game is reported as that rather than killed.

**`StdLogSinkGodot` drops the context dictionary for warnings and errors.** It hands
`push_warning` the message alone, so anything the reader needs — here the port and the
error — has to be in the message string. `info` and below keep their context.

**The viewport image is read after `await RenderingServer.frame_post_draw`**, per the
milestone 0 finding. Two attempts to make that fail on this machine did not: awaiting
`process_frame` instead still produced a complete frame, and a probe which changed
`ColorRect.color` and captured on both sides of the draw produced two byte-identical
images, because that setter queues its redraw for the *next* frame. So the await is kept
as recorded but is unproven here; treat it as cheap insurance rather than a demonstrated
requirement. Every awaiting command also carries a `SceneTreeTimer` watchdog, which is
demonstrable: a coroutine that never resumes takes its reply with it and leaves the
client waiting on a socket nothing will ever write to.

### The game's errors are the point
...
also catches what an in-engine `Logger` tap could not: an error raised before the bridge
mounts, and a crash. `logs` tails it through the same noise filter the edit hook uses,
and `wait` scans it as it polls, so a game that dies during boot reports
...
command that wrote to it, so scanning it whole meant one stale line — a screenshot
written to an unwritable path an hour earlier, say — made every later `wait` abandon a
perfectly healthy game citing an error it had long since recovered from. `launch`
truncates the log, so its own wait reads the whole run.
...
One line in a healthy log looks worse than it is: `stop` sends the notification a window
close sends, and `Lifecycle.shutdown()` answers it by printing `print_orphan_nodes` in
an editor-feature build, so `logs` after a stop can end on `Stray Node: …`. That is the
project's own shutdown diagnostic, not a failure of the run.

### Driving it from a Windows shell

Two things bite in an MSYS or Git Bash shell, neither of them the client's doing. Both
are handled, and the handling is why the two extra pieces exist:
